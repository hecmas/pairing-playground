...
rbin = r.digits(2)
assert Tate(P,DQ) == 33*u^3 + 43*u^2 + 45*u + 39
print(Weil(P,Q,DP,DQ))
# assert Weil(P,D) == 22*u^3 + 12*u^2 + 32*u + 13

# For sure, the pairing is non-degenerate
assert P.additive_order() == Q.additive_order() == r
# assert Weil(P,Q,DP,DQ) != F.one()

# # Let's check the bilinearity of the pairing
# assert Weil(2*P, 12*Q) == Weil(P,12*Q)^2 == Weil(2*P,Q)^12 == Weil(P,D)^24 == Weil(12*P,2*Q)
...
# assert multiplicative_order(Weil(P,Q,DP,DQ)) == r